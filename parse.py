...
def trimContent(content):
  content = re.sub(r'<size=(\d+)>',
    ...
    content)
  content = re.sub(r'<color=(#?\w+)>',
    lambda x : "<font color=\"%s\">" % x.group(1),
    content)
...
def getTime(name):
  ...
  if r:
    ...
    time = int('{:0>3d}{:0>2d}{:0>2d}{:0>2d}'.format(int(r.group(1)), int(r.group(2)), int(r.group(3)), int(num)))
...
def loadIM():
  imlist = getJson('./res/export/assets/game/common/bundleassets/datasheets/imdata/im_topic_data.txt')
  posts = []
  
  for i in imlist:
    replies = []

    post = getJson('./res/export/assets/game/common/bundleassets/datasheets/imdata/imposts/%s/posts.txt' % i['Id'])
    ...
    putJson('./res/converted/data/imposts/%s.json' % i['Id'].lower(), {
      'id': post['Id'].lower(),
...
def loadOS():
  times = []
  oslist = []
  res = getJson('./res/export/assets/game/common/bundleassets/datasheets/cutscenedata/cutscene_data.txt')
  for cfile in os.listdir('./res/export/assets/game/common/bundleassets/datasheets/osdata'):
    chara = cfile.split('.')[0]
    cdata = getJson('./res/export/assets/game/common/bundleassets/datasheets/osdata/%s' % cfile)
    files = []
    ...
    for i in cdata:
      ...
      putJson('./res/converted/data/osfiles/%s/%s.json' % (chara, i['Id']), {
        'id': i['Id'],
        'name': i['Names'][lang[0]],
        'res': (res[i['Id']]['MovieFile'] if i['Id'] in res.keys() else None),
        'contents': contents
      })
      
      # Add to filelist and timelist
      files.append({'id': i['Id'], 'name': i['Names'][0]})
      times.append({'id': i['Id'], 'name': i['Names'][0], 'folder': chara, 'time': time})
...
def loadDB():
  folders = []
  for chara in getJson('./res/export/assets/game/common/bundleassets/datasheets/gallerydata/folder.txt'):
    files = []
    for dbfile in getJson('./res/export/assets/game/common/bundleassets/datasheets/gallerydata/%s.txt' % chara['Id']):
      # NEKO Event Special
      if dbfile["FileLocation"] == "nekoHacked(chaosGlitch)":
        dbfile["FileLocation"] = "story_004"

      # Add to filelist
      files.append({'name': dbfile['FileLocation'],'title': dbfile['FileName'],'type': dbfile["Format"]})
    # Add extra musics
    if chara['Id'] == 'misc':
      for music in os.listdir('./res/export/audios/extra'):
        mname = music.split('.')[0]
        files.append({'name':mname,'title':mname,'type':3})
